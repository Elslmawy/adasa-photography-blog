...
import { siteInfo, posts } from '../data/siteData';

// صفحة التعريف بالموقع (من نحن)
export default function AboutPage() {
  // استخراج قائمة الكتاب الفريدين بدون تكرار من بيانات المقالات
  const authors = [
    ...new Map(posts.map((p) => [p.author.name, p.author])).values(),
  ];

  // أرقام وإحصائيات الموقع
  const stats = [
  ...
  // القيم والمبادئ الأساسية للمنصة
  const values = [
    {
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {stats.map((item, idx) => (
              <div key={idx} className="glass-card p-6">
                <i className={`${item.icon} text-2xl text-orange-500 mb-2 block`} />
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v, idx) => (
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${v.color} opacity-0 group-hover:opacity-10 transition-opacity duration-300`}
                />
                <div className="relative">
                  <i className={`${v.icon} text-4xl text-orange-500 mb-4 block`} />
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#0a0a0a] text-white font-semibold rounded-xl hover:bg-neutral-900 transition-all duration-300 hover:-translate-y-0.5 shadow-xl"
            >
